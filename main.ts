class Isentropic{
  ...
  tstar_by_t(M:number, gamma:number):number{
    return 2/(gamma + 1);
  }
  pstar_by_p(M:number, gamma:number):number{
    const tstar = this.tstar_by_t(M, gamma);
    return Math.pow(tstar, gamma/(gamma - 1));
  }
  rhostar_by_rho(M:number, gamma:number):number{
    const tstar = this.tstar_by_t(M, gamma);
    return Math.pow(tstar, 1/(gamma - 1));
  }
  a_by_astar_pressure(M:number, gamma:number, p:number, p0:number):number{
    const gp = gamma+1;
    const gn = gamma-1;
    const pr = p/p0;
  ...
  a_by_astar_Mach(M:number, gamma: number):number{
